  ...
  template: `
    <div class="mfe-container">
      <div class="page-header">
        ...
        <div>
          <h2>Law Case Details</h2>
          <p class="subtitle" *ngIf="record">{{ record['case_number'] }}</p>
        </div>
...
export class CaseDetailComponent implements OnInit {
  ...
  loadRecord(id: string): void {
    ...
    this.http.get<any>(`http://localhost:8000/api/cases/${id}`, { headers }).subscribe({
      next: (res) => { this.record = res; this.loading = false; },
      error: () => {
        this.record = { id: 1, case_number: 'CASE-2024-001', title: 'Property Dispute Resolution', case_type: 'Civil', priority: 'high', assigned_officer: 'Officer Williams', status: 'open', filed_at: '2024-01-15', description: 'Boundary dispute between neighbors' };
        this.loading = false;
      }
    });
  }
  ...
  updateStatus(status: string): void {
    const token = localStorage.getItem('gov_token');
    const headers: any = token ? { Authorization: `Bearer ${token}` } : {};
    this.http.patch(`http://localhost:8000/api/cases/${this.record.id}`, { status }, { headers }).subscribe({
      next: () => {
