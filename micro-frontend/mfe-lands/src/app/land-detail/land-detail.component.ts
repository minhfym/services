  ...
  template: `
    <div class="mfe-container">
      <div class="page-header">
        ...
        <div>
          <h2>Land Administration Details</h2>
          <p class="subtitle" *ngIf="record">{{ record['plot_number'] }}</p>
        </div>
...
export class LandDetailComponent implements OnInit {
  ...
  loadRecord(id: string): void {
    ...
    this.http.get<any>(`http://localhost:8000/api/lands/${id}`, { headers }).subscribe({
      next: (res) => { this.record = res; this.loading = false; },
      error: () => {
        this.record = { id: 1, plot_number: 'PLT-001-2024', owner_name: 'John Landowner', location: 'North District, Block A', area: 2500, land_use: 'Residential', status: 'active', registered_at: '2023-06-01' };
        this.loading = false;
      }
    });
  }
  ...
  updateStatus(status: string): void {
    const token = localStorage.getItem('gov_token');
    const headers: any = token ? { Authorization: `Bearer ${token}` } : {};
    this.http.patch(`http://localhost:8000/api/lands/${this.record.id}`, { status }, { headers }).subscribe({
      next: () => {
