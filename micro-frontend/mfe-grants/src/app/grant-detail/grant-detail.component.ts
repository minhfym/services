  ...
  template: `
    <div class="mfe-container">
      <div class="page-header">
        ...
        <div>
          <h2>Grants Management Details</h2>
          <p class="subtitle" *ngIf="record">{{ record['grant_code'] }}</p>
        </div>
...
export class GrantDetailComponent implements OnInit {
  ...
  loadRecord(id: string): void {
    ...
    this.http.get<any>(`http://localhost:8000/api/grants/${id}`, { headers }).subscribe({
      ...
      error: () => {
        this.record = { id: 1, grant_code: 'GRT-2024-001', title: 'Small Business Development Grant', category: 'Business', amount: 25000, deadline: '2024-12-31', status: 'open', description: 'Supporting small business growth' };
        this.loading = false;
      }
    });
  }
  ...
  updateStatus(status: string): void {
    const token = localStorage.getItem('gov_token');
    const headers: any = token ? { Authorization: `Bearer ${token}` } : {};
    this.http.patch(`http://localhost:8000/api/grants/${this.record.id}`, { status }, { headers }).subscribe({
      next: () => {
