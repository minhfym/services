  ...
  template: `
    <div class="mfe-container">
      <div class="page-header">
        ...
        <div>
          <h2>Citizen Registration Details</h2>
          <p class="subtitle" *ngIf="record">{{ record['registration_number'] }}</p>
        </div>
...
export class RegistrationDetailComponent implements OnInit {
  ...
  loadRecord(id: string): void {
    ...
    this.http.get<any>(`http://localhost:8000/api/registrations/${id}`, { headers }).subscribe({
      next: (res) => { this.record = res; this.loading = false; },
      error: () => {
        this.record = { id: 1, registration_number: 'REG-2024-001', citizen_name: 'Alice Newborn', registration_type: 'Birth Certificate', national_id: 'N/A', status: 'approved', submitted_at: '2024-01-20', notes: 'Birth registration for newborn' };
        this.loading = false;
      }
    });
  }
  ...
  updateStatus(status: string): void {
    const token = localStorage.getItem('gov_token');
    const headers: any = token ? { Authorization: `Bearer ${token}` } : {};
    this.http.patch(`http://localhost:8000/api/registrations/${this.record.id}`, { status }, { headers }).subscribe({
      next: () => {
